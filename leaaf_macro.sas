...
%macro separate(start, end);
	%do i=&start %to &end;
		...
		data bootdata.bootsample_r_&i;
			set bootdata.bootsample_r0(where=(sampleid=&i));
		run;
...
%macro jackdata(start, end);
	/* use macro for %do processing utility; start and end variables to enable parallel processing */
%do i=&start %to &end;
...
%macro bca_debug(libboot, libjack, cond, labcond, numboot);
	...
	data boot_leaaf_combined_complete;
		set &libboot..boot_leaaf_COMBINED_all end=eof;
	...
	data bootdist(keep=sample mean_col&cond) bias_col&cond(keep=bias propless);
		...
		if mean_col&cond lt orig_leaaf then
			lessthan=1; /* flag if bootstrap sample gives lower estimate */
		else
			lessthan=0;
		retain nless 0;

		if sample gt 0 then
			nless=nless+lessthan; /* count samples with flag for less than comparison with original estimate */
		if sample ne 0 then
			output bootdist;

		/* bootstrap le_aaf */
		if eof then
			do; /* for the last value calculate: */
				...
				output bias_col&cond; /* 3. output only that record to new data set */
			end;
	run;
	...
	data jack_leaaf_combined_complete;
		set &libjack..jack_leaaf_COMBINED_all;
	...
	proc sql noprint;
		select mean(jmean_col&cond) /* put mean of jackknifed values into macro variable */
			into :meanjack1 from jacksum;
	quit;

	data jacksum2;
		set jacksum;
		cubed=(&meanjack1 - jmean_col&cond)**3; /* create cubed value of difference */
		squared=(&meanjack1 - jmean_col&cond)**2; /* create squared value of difference */
	run;
	...
	data accel_col&cond;
		set jacksum3;
		accel=sumcube / (6 * (sumsquar**1.5)); /* plug values into equation for the acceleration statistic */
		keep accel;
	run;
	...
	data ciends;
	   ...
	   bias_col&cond;
		...
		n1=alpha1*&numboot;
		n2=alpha2*&numboot;
		/* Create macro variables with values of N1 and N2 for later use */
	...
	data ci_bca;
		...
		/* select values for upper and lower limits using N1 and N2 values */
		if _n_=&n1 then
